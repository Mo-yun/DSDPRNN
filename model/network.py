...
from .module import Encoder, Decoder, DPRNN_ME
EPS = 1e-8


class Time_DSDPRNN(nn.Module):
    def __init__(self, N, L, B, H, R, K, rnn_type='LSTM', stride=None, dropout=0, bidirectional=False, encoder_activation='relu', mask=True, mask_nonlinear='relu'):
        ...
        self.mask = mask
        self.encoder_noisy = Encoder(N, L, stride=stride, activation=encoder_activation)
        self.encoder_refer = Encoder(N, L, stride=stride, activation=encoder_activation)
    ...
    def forward(self, noisy, refer, inference=False):
        ...
        if self.mask:
            clean_en = clean * noisy_en
        else:
            clean_en = clean
        clean = self.decoder_audio(clean_en)
        clean = clean[:, :length]
        return clean
